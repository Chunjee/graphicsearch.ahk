...
class FindText {
	...
	find(x1:=0, y1:=0, x2:=0, y2:=0, err1:=0, err0:=0
	...
	; Sort the results returned by find() from left to right
	...
	Gui(cmd, arg1:="")
	{
		...
		Case "MakeMainWindow":
			...
			Gui, Add, Checkbox, x+30 ys vAddFunc, % Lang["AddFunc"] " find()"
		...
		Case "OK","SplitAdd","AllAdd":
			...
			s:="`graphicsearch_query := ""|<" Comment ">" color "$" txt """"
			clipboard := s
			if (cmd="AllAdd")
			{
				Event:=cmd, Result:=s
